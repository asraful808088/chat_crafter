...
function checkValidityNER(items) {
  ...
  function sentMapper(mainWord, ENpart2, withOutB = false) {
    ...
    for (let index = 0; index < mainWord.length; index++) {
        ...
        if (index === 0) {
            ...
            entitywordList.push(ENpart2[0].startsWith("B-") ? "J-" + ENpart2[0].slice(2) : ENpart2[0]);
        }
    }
  ...
  function sentMaping(alternative, sentmap) {
    ...
    return [newCreateItems,entitiesItems]
  }


// Original sentence (split into words)
let sentmap = [
    ["I", "love", "New", "York","Ss"], 
    ["O", "O", "B-LOC", "J-LOC","O"],  
    ["I", "love", "New", "York","Ss"]   
  ...
  // Alternative words to replace "New York"
  let alternative = [
    {
      targetIndex: [3,4],  
      list_of_alter: [
        { 
          mainWord: "Los Angeles", 
          alternative: ["San Francisco asd", "Miami asd","Miami asd asd asd","Miami asd asdas asd asd","Miami asd asd asd asd asd asd"]
        }
      ]
    }
  ];
  
  // Call sentMaping to generate alternative sentences
  let result = sentMaping(alternative, sentmap);

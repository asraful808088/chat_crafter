...
class CovController():
    def __init__(self,scriptName = None,m={
        ...
        if scriptName == None:
            try:
                file_path = os.path.join(current_file_path.replace("controller.py",""), "scripts", "${name}.json")
...
// export function createControllerBuilder(name,model_name = "",target_rate =.9){
...
export function createChatHandel2(){
   const code = `
...
export function createChatHandel() {
  const code = `
